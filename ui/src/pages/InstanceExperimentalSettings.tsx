...
export function InstanceExperimentalSettings() {
  ...
  useEffect(() => {
    setBreadcrumbs([
      ...
      { label: t("experimentalSettings.title") },
    ]);
  }, [setBreadcrumbs, t]);
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("experimentalSettings.failedToUpdate"));
    },
  });
    ...
    return (
      <div className="text-sm text-destructive">
        {experimentalQuery.error instanceof Error
          ...
          : t("experimentalSettings.failedToLoad")}
      </div>
    );
  }
    ...
    <div className="max-w-4xl space-y-6">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">
          {t("experimentalSettings.description")}
        </p>
      </div>
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("experimentalSettings.enableIsolatedWorkspaces")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("experimentalSettings.enableIsolatedWorkspacesDescription")}
            </p>
          </div>
          <button
            type="button"
      ...
      <section className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-start justify-between gap-4">
          <div className="space-y-1.5">
            <h2 className="text-sm font-semibold">{t("experimentalSettings.autoRestartDevServer")}</h2>
            <p className="max-w-2xl text-sm text-muted-foreground">
              {t("experimentalSettings.autoRestartDevServerDescription")}
            </p>
          </div>
          <button
            type="button"
