  ...
  return (
    <div className="rounded-lg border border-border p-4 space-y-4">
      <div className="flex items-center justify-between">
        ...
        <span className="text-xs text-muted-foreground">
          {trigger.kind === "schedule" && trigger.nextRunAt
            ? `${t("routineDetail.next")}: ${new Date(trigger.nextRunAt).toLocaleString()}`
      ...
      <div className="flex flex-wrap items-center gap-2">
        {trigger.lastResult && <span className="text-xs text-muted-foreground">{t("routineDetail.last")}: {trigger.lastResult}</span>}
...
export function RoutineDetail() {
  ...
  const saveRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        title: t("routineDetail.failedToSaveRoutine"),
        body: error instanceof Error ? error.message : t("routineDetail.failedToSaveRoutineBody"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routines.routineRunFailedBody"),
        tone: "error",
      });
    },
  });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (error) => {
      pushToast({
        ...
        body: error instanceof Error ? error.message : t("routines.failedToUpdateRoutineBody"),
        tone: "error",
      });
    },
  });
  ...
  const automationLabel = routine.status === "archived" ? t("routines.archived") : automationEnabled ? t("routineDetail.active") : t("routineDetail.paused");
      ...
      {secretMessage && (
        <div className="rounded-lg border border-blue-500/30 bg-blue-500/5 p-4 space-y-3 text-sm">
          <div>
            ...
            <p className="text-xs text-muted-foreground">{t("routineDetail.saveSecretWarning")}</p>
          </div>
