  ...
  return (
    <div className="relative w-48 sm:w-64 md:w-80">
      ...
      <Input
        ...
        placeholder={t("components.issuesList.searchIssues")}
...
export function IssuesList({
  ...
  const groupedContent = useMemo(() => {
    ...
    return Object.keys(groups).map((key) => ({
      ...
      label:
        key === "__unassigned"
          ? t("components.issuesList.unassigned")
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between gap-2 sm:gap-3">
        <div className="flex min-w-0 items-center gap-2 sm:gap-3">
          <Button size="sm" variant="outline" onClick={() => openNewIssue(newIssueDefaults())}>
            ...
            <span className="hidden sm:inline">{t("components.issuesList.newIssue")}</span>
          </Button>
          <IssuesSearchInput
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          <div className="flex items-center border border-border rounded-md overflow-hidden mr-1">
            <button
              ...
              title={t("components.issuesList.listView")}
            >
            ...
            <button
              ...
              title={t("components.issuesList.boardView")}
            >
          ...
          <Popover>
            <PopoverTrigger asChild>
              <Button variant="ghost" size="sm" className={`text-xs ${activeFilterCount > 0 ? "text-blue-600 dark:text-blue-400" : ""}`}>
                ...
                <span className="hidden sm:inline">{activeFilterCount > 0 ? t("components.issuesList.filtersCount", { count: activeFilterCount }) : t("components.issuesList.filter")}</span>
                {activeFilterCount > 0 && (
            ...
            <PopoverContent align="end" className="w-[min(480px,calc(100vw-2rem))] p-0">
              <div className="p-3 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm font-medium">{t("components.issuesList.filters")}</span>
                  {activeFilterCount > 0 && (
                    <button
                      className="text-xs text-muted-foreground hover:text-foreground"
                      onClick={() => updateView({ statuses: [], priorities: [], assignees: [], labels: [] })}
                    >
                      {t("components.issuesList.clear")}
                    </button>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  <span className="text-xs text-muted-foreground">{t("components.issuesList.quickFilters")}</span>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 gap-y-3">
                  ...
                  <div className="space-y-1">
                    <span className="text-xs text-muted-foreground">{t("components.issuesList.status")}</span>
                  ...
                  <div className="space-y-3">
                    {/* Priority */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("components.issuesList.priority")}</span>
                      <div className="space-y-0.5">
                        {priorityOrder.map((p) => (
                    ...
                    {/* Assignee */}
                    <div className="space-y-1">
                      <span className="text-xs text-muted-foreground">{t("components.issuesList.assignee")}</span>
                      <div className="space-y-0.5 max-h-32 overflow-y-auto">
                        <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                          ...
                          />
                          <span className="text-sm">{t("components.issuesList.noAssignee")}</span>
                        </label>
                        {currentUserId && (
                          <label className="flex items-center gap-2 px-2 py-1 rounded-sm hover:bg-accent/50 cursor-pointer">
                            <Checkbox
                            ...
                            <span className="text-sm">{t("components.issuesList.me")}</span>
                          </label>
                        )}
                        {(agents ?? []).map((agent) => (
                    ...
                    {labels && labels.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("components.issuesList.labels")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
                          {labels.map((label) => (
                    ...
                    {projects && projects.length > 0 && (
                      <div className="space-y-1">
                        <span className="text-xs text-muted-foreground">{t("components.issuesList.project")}</span>
                        <div className="space-y-0.5 max-h-32 overflow-y-auto">
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("components.issuesList.sort")}</span>
                </Button>
              </PopoverTrigger>
            ...
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="ghost" size="sm" className="text-xs">
                  ...
                  <span className="hidden sm:inline">{t("components.issuesList.group")}</span>
                </Button>
              </PopoverTrigger>
      ...
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message={t("components.issuesList.noIssuesMatch")}
          action={t("components.issuesList.createIssue")}
                      ...
                      >
                          ...
                          >
                            {issue.assigneeAgentId && agentName(issue.assigneeAgentId) ? (
                              ...
                              <span className="inline-flex items-center gap-1.5 text-xs text-muted-foreground">
                                ...
                                {t("components.issuesList.assignee")}
                              </span>
                            )}
                          </button>
                        </PopoverTrigger>
                        ...
                        >
                          <input
                            ...
                            placeholder={t("components.issuesList.searchAssignees")}
                          ...
                          <div className="max-h-48 overflow-y-auto overscroll-contain">
                            ...
                            >
                              {t("components.issuesList.noAssignee")}
                            </button>
                            {currentUserId && (
                              <button
                                className={cn(
