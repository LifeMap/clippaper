...
function CopyMarkdownButton({ text }: { text: string }) {
  ...
  return (
    <button
      ...
      title={t("components.commentThread.copyAsMarkdown")}
      onClick={() => {
    ...
    >
      <div className="flex items-center justify-between mb-1">
        {comment.authorAgentId ? (
          ...
          <Identity name={t("components.commentThread.you")} size="sm" />
        )}
        <span className="flex items-center gap-1.5">
          {isQueued ? (
            <span className="inline-flex items-center rounded-full border border-amber-400/60 bg-amber-100/70 px-2 py-0.5 text-[10px] font-medium uppercase tracking-[0.14em] text-amber-800 dark:border-amber-400/40 dark:bg-amber-500/20 dark:text-amber-200">
              {t("components.commentThread.queued")}
            </span>
          ) : null}
          {companyId && !isPending ? (
            <PluginSlotOutlet
          ...
          {isPending ? (
            <span className="text-xs text-muted-foreground">{isQueued ? t("components.commentThread.queueing") : t("components.commentThread.sending")}</span>
          ) : (
            <a
...
const TimelineList = memo(function TimelineList({
  ...
  if (timeline.length === 0) {
    return <p className="text-sm text-muted-foreground">{t("components.commentThread.noCommentsOrRuns")}</p>;
  }
          ...
          return (
            <div key={`run:${run.runId}`} className="border border-border bg-accent/20 p-3 overflow-hidden min-w-0 rounded-sm">
              ...
              <div className="flex items-center gap-2 text-xs">
                <span className="text-muted-foreground">{t("components.commentThread.run")}</span>
                <Link
...
export function CommentThread({
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-sm font-semibold">{t("components.commentThread.commentsAndRuns", { count: timeline.length + queuedComments.length })}</h3>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between gap-2">
            <h4 className="text-xs font-semibold uppercase tracking-[0.14em] text-amber-700 dark:text-amber-300">
              {t("components.commentThread.queuedComments", { count: queuedComments.length })}
            </h4>
            {onInterruptQueued && queuedComments[0]?.queueTargetRunId ? (
              ...
              >
                {interruptingQueuedRunId === queuedComments[0].queueTargetRunId ? t("components.commentThread.interrupting") : t("components.commentThread.interrupt")}
              </Button>
            ) : null}
          </div>
          <div className="space-y-3">
      ...
      <div className="space-y-2">
        <MarkdownEditor
          ...
          value={body}
          onChange={setBody}
          placeholder={t("components.commentThread.leaveComment")}
          mentions={mentions}
        ...
        <div className="flex items-center justify-end gap-3">
          {(imageUploadHandler || onAttachImage) && (
            <div className="mr-auto flex items-center gap-3">
              ...
              <Button
                ...
                disabled={attaching}
                title={t("components.commentThread.attachImage")}
              >
          ...
          <label className="flex items-center gap-1.5 text-xs text-muted-foreground cursor-pointer select-none">
            ...
            />
            {t("components.commentThread.reopen")}
          </label>
          ...
          <Button size="sm" disabled={!canSubmit} onClick={handleSubmit}>
            {submitting ? t("components.commentThread.posting") : t("components.commentThread.comment")}
          </Button>
        </div>
      </div>
    </div>
