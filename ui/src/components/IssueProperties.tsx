...
export function IssueProperties({ issue, onUpdate, inline }: IssuePropertiesProps) {
  ...
  const labelsTrigger = (issue.labels ?? []).length > 0 ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("components.issueProperties.noLabels")}</span>
    </>
  );
  ...
  const labelsContent = (
    <>
      <input
        ...
        placeholder={t("components.issueProperties.searchLabels")}
        value={labelSearch}
      ...
      <div className="mt-2 border-t border-border pt-2 space-y-1">
        <div className="flex items-center gap-1">
          ...
          <input
            ...
            placeholder={t("components.issueProperties.newLabel")}
            value={newLabelName}
        ...
        >
          ...
          {createLabel.isPending ? t("components.issueProperties.creating") : t("components.issueProperties.createLabel")}
        </button>
      </div>
    </>
  );
  ...
  const assigneeTrigger = assignee ? (
    ...
    <>
      <User className="h-3.5 w-3.5 text-muted-foreground" />
      <span className="text-sm text-muted-foreground">{t("components.issueProperties.unassigned")}</span>
    </>
  );
  ...
  const assigneeContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("components.issueProperties.searchAssignees")}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        >
          {t("components.issueProperties.noAssignee")}
        </button>
        {currentUserId && (
          <button
            className={cn(
          ...
          >
            ...
            {t("components.issueProperties.assignToMe")}
          </button>
        )}
        {issue.createdByUserId && issue.createdByUserId !== currentUserId && (
          ...
          >
            <User className="h-3 w-3 shrink-0 text-muted-foreground" />
            {creatorUserLabel ? t("components.issueProperties.assignTo", { name: creatorUserLabel }) : t("components.issueProperties.assignToRequester")}
          </button>
        )}
        {sortedAgents
          .filter((a) => {
  ...
  const projectTrigger = issue.projectId ? (
    ...
    <>
      ...
      <span className="text-sm text-muted-foreground">{t("components.issueProperties.noProject")}</span>
    </>
  );
  ...
  const projectContent = (
    <>
      <input
        className="w-full px-2 py-1.5 text-xs bg-transparent outline-none border-b border-border mb-1 placeholder:text-muted-foreground/50"
        placeholder={t("components.issueProperties.searchProjects")}
        value={projectSearch}
      ...
      <div className="max-h-48 overflow-y-auto overscroll-contain">
        ...
        >
          {t("components.issueProperties.noProject")}
        </button>
        {orderedProjects
          .filter((p) => {
  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("components.issueProperties.status")}>
          <StatusIcon
            status={issue.status}
        ...
        <PropertyRow label={t("components.issueProperties.priority")}>
          <PriorityIcon
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueProperties.labels")}
          open={labelsOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueProperties.assignee")}
          open={assigneeOpen}
        ...
        <PropertyPicker
          inline={inline}
          label={t("components.issueProperties.project")}
          open={projectOpen}
        ...
        {issue.parentId && (
          <PropertyRow label={t("components.issueProperties.parent")}>
            <Link
        ...
        {issue.requestDepth > 0 && (
          <PropertyRow label={t("components.issueProperties.depth")}>
      ...
      <div className="space-y-1">
        {(issue.createdByAgentId || issue.createdByUserId) && (
          <PropertyRow label={t("components.issueProperties.createdBy")}>
        ...
        {issue.startedAt && (
          <PropertyRow label={t("components.issueProperties.started")}>
            <span className="text-sm">{formatDate(issue.startedAt)}</span>
          </PropertyRow>
        )}
        {issue.completedAt && (
          <PropertyRow label={t("components.issueProperties.completed")}>
            <span className="text-sm">{formatDate(issue.completedAt)}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("components.issueProperties.created")}>
          <span className="text-sm">{formatDate(issue.createdAt)}</span>
        </PropertyRow>
        <PropertyRow label={t("components.issueProperties.updated")}>
