  ...
  return (
    <div>
      <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide text-muted-foreground">
        {t("components.activeAgentsPanel.agents")}
      </h3>
      {runs.length === 0 ? (
        <div className="rounded-xl border border-border p-4">
          <p className="text-sm text-muted-foreground">{t("components.activeAgentsPanel.noRecentRuns")}</p>
        </div>
      ) : (
    ...
    <div className={cn(
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0">
            ...
            <div className="mt-2 flex items-center gap-2 text-[11px] text-muted-foreground">
              <span>{isActive ? t("components.activeAgentsPanel.liveNow") : run.finishedAt ? `${t("components.activeAgentsPanel.finished")} ${relativeTime(run.finishedAt)}` : `${t("components.activeAgentsPanel.started")} ${relativeTime(run.createdAt)}`}</span>
            </div>
          </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto p-3">
        <RunTranscriptView
          ...
          emptyMessage={hasOutput ? t("components.activeAgentsPanel.waitingTranscript") : isActive ? t("components.activeAgentsPanel.waitingOutput") : t("components.activeAgentsPanel.noTranscript")}
        />
      </div>
    </div>
  );
