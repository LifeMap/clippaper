  ...
  return (
    <div className="space-y-4">
      <div className="space-y-1">
        <PropertyRow label={t("components.agentProperties.status")}>
          <StatusBadge status={agent.status} />
        </PropertyRow>
        <PropertyRow label={t("components.agentProperties.role")}>
          <span className="text-sm">{roleLabels[agent.role] ?? agent.role}</span>
        </PropertyRow>
        {agent.title && (
          <PropertyRow label={t("components.agentProperties.title")}>
            <span className="text-sm">{agent.title}</span>
          </PropertyRow>
        )}
        <PropertyRow label={t("components.agentProperties.adapter")}>
      ...
      <div className="space-y-1">
        {(runtimeState?.sessionDisplayId ?? runtimeState?.sessionId) && (
          <PropertyRow label={t("components.agentProperties.session")}>
        ...
        {runtimeState?.lastError && (
          <PropertyRow label={t("components.agentProperties.lastError")}>
            <span className="text-xs text-red-600 dark:text-red-400 truncate max-w-[160px]">{runtimeState.lastError}</span>
          </PropertyRow>
        )}
        {agent.lastHeartbeatAt && (
          <PropertyRow label={t("components.agentProperties.lastHeartbeat")}>
            <span className="text-sm">{formatDate(agent.lastHeartbeatAt)}</span>
          </PropertyRow>
        )}
        {agent.reportsTo && (
          <PropertyRow label={t("components.agentProperties.reportsTo")}>
            {reportsToAgent ? (
        ...
        <PropertyRow label={t("components.agentProperties.created")}>
